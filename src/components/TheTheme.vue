...
// mode list
interface modeListType { icon: string; name: string; }
  ...
  <Popover>
    <PopoverTrigger>
      <Button variant="outline">
          ...
          <span class="h-4 w-4 rounded-full flex items-center justify-center ml-2" :class="color[theme]"></span>
      </Button>
    </PopoverTrigger>
    ...
    <PopoverContent  class="w-100">
      <div>
        ...
        <div class="grid grid-cols-3 gap-2 py-1.5">
          <Button v-for="_mode in modeList" :key="_mode.name" :class="mode === _mode.name.toLocaleLowerCase()  ? 'border-2 border-foreground' : ''" class="h-8" variant="outline" @click="toggleMode(_mode.name.toLocaleLowerCase())">
        ...
        <div class="grid grid-cols-3 gap-2 py-1.5">
          <Button
            ...
            :key="_theme"
            :class="theme === 'theme-' + _theme.toLocaleLowerCase()  ? 'border-2 border-foreground' : ''" 
            @click="changeTheme('theme-' + _theme.toLocaleLowerCase())" 
            variant="outline"
            class="h-8 justify-start"
          >
            <span class="h-4 w-4 rounded-full flex items-center justify-center" :class="'bg-' + _theme.toLocaleLowerCase()"></span>
